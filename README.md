...
- **Celery + Redis + APScheduler** — background workers plus in-backend scheduler for automatic live-game analysis
...
| `ANALYSIS_SCHEDULER_ENABLED` | enable APScheduler tick loop in Django backend (`false` by default) |
| `ANALYSIS_SCHEDULER_INTERVAL_SECONDS` | APScheduler interval in seconds (default `1`) |
...
Background analysis for active games is triggered by APScheduler inside the Django backend service (`ANALYSIS_SCHEDULER_ENABLED=true`); workers consume those analysis tasks.
