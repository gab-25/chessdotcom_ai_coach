...
| Coach | [`services/coach.py`](../chessdotcom_ai_coach/services/coach.py) | `get_best_move(fen, pgn)` → a `Suggestion` TypedDict. Stockfish first (2s, asked for the score **and** the principal variation), then OpenRouter (60s timeout); on LLM error it returns Stockfish-only prose rather than failing. The prompt carries the board as a labelled grid, the side to move, material, castling rights and the engine's main line — a model given only a FEN and a move invents the reason the move is good. |
