...
# gunicorn worker): migrations above have already created the tables it reads.
# It's a background process, independent of gunicorn's worker forks, so exactly
# one scheduler instance exists per container — which matters, because its job is
# importing Chess.com archives and a second copy would fetch every month twice.
